...
public class LlmIntentWorker {
    ...
    public ChatCompletionResult process(ChatCompletionRequest chatCompletionRequest, String url) {
        ...
        // 将提示词整合进请求
        String system_prompt = "# 角色\n角色：你的任务是识别用户的意图。你所在的场景是福州机场，你扮演的是一名机场工作人员。" +
                "你要将用户的问题归类到以下几个类别中：机场相关类(airport_related)、非机场相关类(non-airport_related)\n" +
                "机场相关次要类别:" +
                "- 航班状态类(flight_status)\n,只能包含询问目的地、时间段、航班号信息以及想要去某个城市的想法。不包括询问飞机内服务、登机口位置、航班起降情况的、询问飞机机体信息意图。 " +
                "- 识别是否查询机场内部地图导航的问题(airport_map),包含并不限于登机口、行李寄存处、到达大厅、国际出发大厅、厕所、吸烟室等位置的查询；餐饮、便利店等商铺；通过商品推荐相关商户，如土特产，小电器、书籍等找到相关的商户;自助值机柜台、航司值机柜台可以为旅客办理托运及打印登机牌的业务.\n  " +
                "- 乘机流程及相关规定的查询(airport_rules)：如中国民航局的统一乘机规定，或者各航空公司对乘机、托运、随身携带物品，年龄性别限制等规定的查询；比如飞机行驶规范的咨询。(airport_rules)\n " +
                "非机场相关次要类别:\n" +
